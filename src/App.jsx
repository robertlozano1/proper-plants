...
export default function App() {
  // Requirement 3: Cart is managed as a state variable in <App>
  const [cart, setCart] = useState([]);

  // Requirement 4 & 5: Functions to add/remove items from cart, using array methods to avoid mutation
  const addToCart = (plant) => {
    setCart((currentCart) => {
      ...
      if (existingItem) {
        // Requirement 7: If plant is already in cart, increase quantity by 1
        // Using map to create a new array (no mutation)
        return currentCart.map((item) =>
          item.id === plant.id ? { ...item, quantity: item.quantity + 1 } : item
        );
      } else {
        // Requirement 7: If plant is not in cart, add it with quantity of 1
  ...
  const updateQuantity = (plantId, newQuantity) => {
    setCart((currentCart) => {
      // Requirement 13: If quantity is 0 or below, remove item from cart
      if (newQuantity <= 0) {
  ...
  return (
    <div className="app">
      ...
      <div className="main-container">
        {/* Requirement 6 & 8: Display all plants, organized in separate components */}
        <div className="plants-section">
          <h2>Plants</h2>
          <PlantList plants={PLANTS} onAddToCart={addToCart} />
        </div>

        {/* Requirement 9 & 14: Cart renders cart items, organized in separate components */}
